...
__kernel void ThermalPropagation(__read_only image2d_t input_image, __write_only image2d_t output_image,__constant float * data, sampler_t sampler){
	...
	float tempScale = data[6];
	float ratio = 0.0F;
	float extTransfer = 0.0001F;
	...
	if(coords.x == 0 || coords.y == 0 || coords.x == width - 1 || coords.y == height - 1){
		write_imagef(output_image, coords, (float4)(extTemp/tempScale, 0.0F, 0.0F, 0.0F));
		return;
	}

	if(coords.x == sourceCoords.x && coords.y == sourceCoords.y){
		write_imagef(output_image, coords, (float4)(sourceTemp/tempScale, 0.0F, 0.0F, 0.0F));
		return;
	}
